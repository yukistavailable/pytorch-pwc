...
def backwarp(tenInput, tenFlow):
    ...
    tenFlow = torch.cat([tenFlow[:,
                                 0:1,
                                 :,
                                 :] / ((tenInput.shape[3] - 1.0) / 2.0),
                         tenFlow[:,
                                 1:2,
                                 :,
...
def estimate(tenOne, tenTwo):
    ...
    # remember that there is no guarantee for correctness, comment this line
    # out if you acknowledge this and want to continue
    ...
    tenFlow = torch.nn.functional.interpolate(
        input=netNetwork(
            tenPreprocessedOne, tenPreprocessedTwo), size=(
...
if __name__ == '__main__':
    tenOne = torch.FloatTensor(
        numpy.ascontiguousarray(
            numpy.array(
                PIL.Image.open(arguments_strOne))[
                :,
                :,
                ::-
                1].transpose(
    ...
    tenTwo = torch.FloatTensor(
        numpy.ascontiguousarray(
            numpy.array(
                PIL.Image.open(arguments_strTwo))[
                :,
                :,
                ::-
                1].transpose(
